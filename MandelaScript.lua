-- ==============================================================================
-- SPEED CONTROL PANEL V3.5 (SPECIAL ANTI-CHEAT EGG BYPASS)
-- ==============================================================================
-- Compatible con: Mobile & PC
-- Solución al error: "delivery failed the egg was returned to its nest"
-- ==============================================================================local TweenService = game:GetService("TweenService")
local UserInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
local Players = game:GetService("Players")local LocalPlayer = Players.LocalPlayer-- Determinar el contenedor seguro para la interfaz
local ParentGui
...
end-- Limpiar versiones anteriores
...
ScreenGui.Parent = ParentGui-- Panel Principal
...
MainStroke.Parent = MainFrame-- Barra Superior
...
TitleCorner.Parent = TitleBarlocal TitleFix = Instance.new("Frame")
TitleFix.Size = UDim2.new(1, 0, 0, 12)
TitleFix.Position = UDim2.new(0, 0, 1, -12)
TitleFix.BackgroundColor3 = Color3.fromRGB(24, 28, 42)
TitleFix.BorderSizePixel = 0
TitleFix.Parent = TitleBarlocal TitleLabel = Instance.new("TextLabel")
...
TitleLabel.Parent = TitleBarlocal CloseBtn = Instance.new("TextButton")
...
CloseBtn.Parent = TitleBarlocal CloseCorner = Instance.new("UICorner")
CloseCorner.CornerRadius = UDim.new(0, 8)
CloseCorner.Parent = CloseBtn-- Botón Flotante para Móvil y PC
...
FloatingToggle.Parent = ScreenGuilocal ToggleCorner = Instance.new("UICorner")
ToggleCorner.CornerRadius = UDim.new(1, 0)
ToggleCorner.Parent = FloatingTogglelocal Content = Instance.new("Frame")
Content.Size = UDim2.new(1, -24, 1, -56)
Content.Position = UDim2.new(0, 12, 0, 50)
Content.BackgroundTransparency = 1
Content.Parent = MainFrame-- Display de Velocidad Normal
local DisplayFrame = Instance.new("Frame")
DisplayFrame.Size = UDim2.new(1, 0, 0, 52)
DisplayFrame.BackgroundColor3 = Color3.fromRGB(10, 12, 20)
DisplayFrame.Parent = Contentlocal DisplayCorner = Instance.new("UICorner")
DisplayCorner.CornerRadius = UDim.new(0, 10)
DisplayCorner.Parent = DisplayFramelocal SpeedTitle = Instance.new("TextLabel")
...
SpeedTitle.Parent = DisplayFramelocal SpeedValLabel = Instance.new("TextLabel")
...
SpeedValLabel.Parent = DisplayFramelocal InputBox = Instance.new("TextBox")
...
InputBox.Parent = DisplayFramelocal InputCorner = Instance.new("UICorner")
InputCorner.CornerRadius = UDim.new(0, 8)
InputCorner.Parent = InputBox-- Slider
...
SliderBack.Parent = Contentlocal SliderCorner = Instance.new("UICorner")
SliderCorner.CornerRadius = UDim.new(1, 0)
SliderCorner.Parent = SliderBacklocal SliderFill = Instance.new("Frame")
SliderFill.Size = UDim2.new(0.016, 0, 1, 0)
SliderFill.BackgroundColor3 = Color3.fromRGB(99, 102, 241)
SliderFill.Parent = SliderBacklocal FillCorner = Instance.new("UICorner")
FillCorner.CornerRadius = UDim.new(1, 0)
FillCorner.Parent = SliderFill-- Presets
...
PresetsGrid.Parent = Contentlocal presetValues = {16, 50, 100, 500, 1000}
...
Btn.TextSize = 11
Btn.Parent = PresetsGridlocal BtnCorner = Instance.new("UICorner")
...
-- MODULOS DE BYPASS Y SEGURIDAD (PARA ENTREGAR HUEVOS)
...
EggBypassFrame.Parent = Contentlocal EggCorner = Instance.new("UICorner")
EggCorner.CornerRadius = UDim.new(0, 10)
EggCorner.Parent = EggBypassFramelocal EggStroke = Instance.new("UIStroke")
EggStroke.Color = Color3.fromRGB(34, 197, 94)
EggStroke.Thickness = 1
EggStroke.Parent = EggBypassFramelocal EggTitle = Instance.new("TextLabel")
...
EggTitle.Parent = EggBypassFramelocal EggSub = Instance.new("TextLabel")
...
EggSub.Parent = EggBypassFramelocal EggToggle = Instance.new("TextButton")
...
EggToggle.Parent = EggBypassFramelocal EggToggleCorner = Instance.new("UICorner")
EggToggleCorner.CornerRadius = UDim.new(1, 0)
EggToggleCorner.Parent = EggToggle-- Ajuste de Velocidad Segura para el Huevo
...
SafeSpeedFrame.Parent = Contentlocal SafeCorner = Instance.new("UICorner")
SafeCorner.CornerRadius = UDim.new(0, 8)
SafeCorner.Parent = SafeSpeedFramelocal SafeLabel = Instance.new("TextLabel")
...
SafeLabel.Parent = SafeSpeedFramelocal SafeInput = Instance.new("TextBox")
...
SafeInput.Parent = SafeSpeedFramelocal SafeInputCorner = Instance.new("UICorner")
SafeInputCorner.CornerRadius = UDim.new(0, 6)
SafeInputCorner.Parent = SafeInput-- Bucle Infinito Normal
...
LoopFrame.Parent = Contentlocal LoopCorner = Instance.new("UICorner")
LoopCorner.CornerRadius = UDim.new(0, 8)
LoopCorner.Parent = LoopFramelocal LoopLabel = Instance.new("TextLabel")
...
LoopLabel.Parent = LoopFramelocal LoopToggle = Instance.new("TextButton")
...
LoopToggle.Parent = LoopFramelocal LoopToggleCorner = Instance.new("UICorner")
LoopToggleCorner.CornerRadius = UDim.new(1, 0)
LoopToggleCorner.Parent = LoopToggle-- ==============================================================================
-- LÓGICA DE CONTROL Y DRAG
-- ==============================================================================local dragging = false
local dragInput, dragStart, startPosTitleBar.InputBegan:Connect(function(input)
...
end)TitleBar.InputChanged:Connect(function(input)
...
end)-- LOGICA DE APLICACIÓN DE VELOCIDAD
local TargetSpeed = 16
local SafeEggSpeed = 22
local EggBypassEnabled = true
local LoopEnabled = truefunction _G.SetSpeed(val)
local num = tonumber(val)
if not num then return end
TargetSpeed = math.clamp(math.floor(num), 1, 1000)SpeedValLabel.Text = tostring(TargetSpeed)
...
end-- Función para comprobar si el jugador lleva un huevo
local function IsCarryingEgg()
if not LocalPlayer.Character then return false end-- 1. Comprobar si tiene una herramienta (Tool) equipada
...
end-- Bucle principal de velocidad
RunService.Stepped:Connect(function()
if not LocalPlayer.Character then return end
local humanoid = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
if not humanoid then return endlocal currentTarget = TargetSpeed
...
local mousePosition = input.Position.Xlocal relativeX = math.clamp(mousePosition - sliderPosition, 0, sliderSize)
local alpha = relativeX / sliderSize
local calculatedSpeed = math.floor(1 + (alpha * 999))

_G.SetSpeed(calculatedSpeed)

endSliderBack.InputBegan:Connect(function(input)
if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
sliding = true
updateSlider(input)
end
...
end)LoopToggle.MouseButton1Click:Connect(function()
...
end)local function ToggleVisibility()
MainFrame.Visible = not MainFrame.Visible
FloatingToggle.Visible = not MainFrame.Visible
endCloseBtn.MouseButton1Click:Connect(ToggleVisibility)
FloatingToggle.MouseButton1Click:Connect(ToggleVisibility)UserInputService.InputBegan:Connect(function(input, gameProcessed)
